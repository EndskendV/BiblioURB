...
namespace WindowsFormsApplication1
{
    public partial class Ventas : Form
    {
        ...
        {
            ConexionVerifing();
            SqlCommand cmd = new SqlCommand(String.Format("SELECT * FROM [Ventas] where [ID_Book]='{0}'", txt_IDBook.Text), conexion);
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            sqlDataAdapter.Fill(dt);
            Grid_Libros.DataSource = dt;
        ...
        private void Combo_Libro_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string temp = "";
            ConexionVerifing();
            SqlCommand sqlCommand = new SqlCommand(String.Format("SELECT ID_Curso,ID_Instructor FROM Curso WHERE NameCurso='{0}'", Combo_Libros.SelectedItem.ToString()), conexion);
            SqlDataReader reader = sqlCommand.ExecuteReader();
            if (reader.Read())
            {
                txt_IDBook.Text = reader.GetString(0);
            ...
            if (reader1.Read())
            {
                txt_Autor.Text = reader1.GetString(0);
            }
            conexion.Close();
        }
        ...
        private void Grupo_Load(object sender, EventArgs e)
        {
            ...
            while (reader.Read())
            {
                Combo_Libros.Items.Add(reader["NameCurso"].ToString());
            }
        }
        ...
        private void texClave_TextChanged(object sender, EventArgs e)
        {

        }
        private void texClave_KeyPress(object sender, PreviewKeyDownEventArgs e)
        {
